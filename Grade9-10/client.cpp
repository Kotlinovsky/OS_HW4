...
int main(int argc, char *argv[]) {
    ...
    // В фоне проверяем приход пакета на закрытие аренды.
    for (int i = 0; i < rent_time_in_seconds; ++i) {
        sleep(1);

        // Если пакет пришел, то закрываем программу.
        std::string close_event_msg(1, 0);
        if (recvfrom(sockfd, close_event_msg.data(), close_event_msg.size(), MSG_DONTWAIT, nullptr, nullptr) == 1 &&
            close_event_msg[0] == 6) {
