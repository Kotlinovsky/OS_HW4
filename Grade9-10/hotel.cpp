...
int main(int argc, char *argv[]) {
    ...
    std::map<std::pair<std::string, int>, std::tuple<uint32_t, Client, Room *, struct sockaddr_in>> clients;
    ...
    while ((client_buffer_length = recvfrom(sockfd, client_buffer.data(), client_buffer.size(), MSG_WAITALL,
        ...
        if (pkg_id == 2) {
            ...
            if (clients.find(address_pair) == clients.end()) {
                continue;
            }
        ...
        // Пакет 4: закрытие отеля.
        if (pkg_id == 4) {
            std::cout << "[SERVER] Terminating server by manager request!" << std::endl;
            std::string disconnect_msg(1, 0);
            writeByteToBuffer(disconnect_msg, 0, 6);

            for (auto &current: clients) {
                sendto(sockfd, disconnect_msg.data(), disconnect_msg.size(), MSG_CONFIRM,
                       (const struct sockaddr *) &std::get<3>(current.second), sizeof(std::get<3>(current.second)));
            }

            break;
        }

        // Пакет 5: отключение клиента.
        if (pkg_id == 5) {
            uint32_t received_client_id = readUInt32FromBuffer(client_buffer, 1);

            // Найдем клиента, отправим ему уведомление и забудем.
            for (auto &current: clients) {
                auto tuple = current.second;
                auto tuple_client_id = std::get<0>(tuple);

                if (tuple_client_id == received_client_id) {
                    ...
                    std::get<2>(tuple)->removeOne();
                    clients.erase(current.first);
                    break;
                }
            }
